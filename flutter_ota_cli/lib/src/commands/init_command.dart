...
/// `flutter_ota init` — wire Flutter OTA into a Flutter app's Android side.
///
...
class InitCommand extends Command<int> {
  ...
  String get description =>
      "Add Flutter OTA's patch loader to the app's Android MainActivity.";
  ...
  Future<int> run() async {
    ...
    if (activity == null) {
      stderr
        ...
        ..writeln('Flutter OTA patches the app’s own Activity, so it needs one.');
      return 1;
    }
      ...
      case PatchOutcome.regenerated:
        if (dryRun) {
          ...
          final verb = result.outcome == PatchOutcome.injected
              ? 'Patched'
              : 'Regenerated the Flutter OTA block in';
    ...
    stdout
      ...
      ..writeln()
      ..writeln("  await FlutterOta.initialize(updateUrl: 'http://10.0.2.2:8080');")
      ..writeln()
      ..writeln('Then build the app, change some Dart, rebuild, and ship it')
      ..writeln('with `flutter_ota patch`. Patches load on the next cold start.');
    return 0;
  }
  ...
  /// The override references `dev.flutterota.FlutterOtaPatch`, which reaches the
  /// app's classpath through the plugin — so the dependency has to be there.
  void _warnIfDependencyMissing(File pubspec) {
    if (pubspec.readAsStringSync().contains('flutter_ota_package')) return;
    stdout
      ..writeln()
      ..writeln('Note: flutter_ota_package is not in ${p.basename(pubspec.path)}.')
      ..writeln('The override will not compile without it. Add:')
      ..writeln()
      ..writeln('  dependencies:')
      ..writeln('    flutter_ota_package:')
      ..writeln('      path: ../flutter_ota_package');
  }
...
'''
  ...
  public io.flutter.embedding.engine.FlutterShellArgs getFlutterShellArgs() {
      return dev.flutterota.FlutterOtaPatch.applyTo(this, super.getFlutterShellArgs());
  }
''';
}
