...
/**
 * Method-channel surface over [FlutterOtaPatch].
 *
 ...
 */
class FlutterOtaPlugin : FlutterPlugin, MethodChannel.MethodCallHandler {
    private lateinit var channel: MethodChannel
    private lateinit var context: Context

    override fun onAttachedToEngine(binding: FlutterPlugin.FlutterPluginBinding) {
        context = binding.applicationContext
        channel = MethodChannel(binding.binaryMessenger, "dev.flutterota/flutter_ota")
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            "state" -> result.success(FlutterOtaPatch.snapshot(context) + appInfo())
            ...
            "stage" -> {
                ...
                if (patchNumber == null || bytes == null || sha256 == null) {
                    ...
                    result.success(FlutterOtaPatch.stage(context, patchNumber, bytes, sha256))
                }
            }

            "confirmLaunch" -> {
                FlutterOtaPatch.confirmLaunch(context)
                result.success(null)
            }

            "rollback" -> {
                FlutterOtaPatch.rollback(context)
                result.success(null)
            }
    ...
    /**
     * The app's identity, so `FlutterOta.initialize` needs nothing but a URL.
     *
