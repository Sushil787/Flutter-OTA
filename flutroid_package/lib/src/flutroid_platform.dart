...
class FlutroidState {
  ...
  /// Inert state for platforms Flutroid does not patch.
  const FlutroidState.unsupported()
    : currentPatch = 0,
      stagedPatch = 0,
      confirmed = false,
      bootAttempts = 0,
  ...
  factory FlutroidState.fromMap(Map<Object?, Object?> map) => FlutroidState(
  ...
  @override
  String toString() =>
      'FlutroidState(current: $currentPatch, staged: $stagedPatch, '
      'confirmed: $confirmed, bootAttempts: $bootAttempts, '
      'active: $patchActive, release: $releaseVersion)';
}

/// Thin wrapper over the `dev.flutroid/flutroid` method channel.
///
/// Every call is a no-op off Android, so an app using Flutroid still runs on
/// the other platforms — it just never patches.
class FlutroidPlatform {
  const FlutroidPlatform._();

  static const MethodChannel _channel = MethodChannel('dev.flutroid/flutroid');
  ...
  static Future<FlutroidState> state() async {
    if (!isSupported) return const FlutroidState.unsupported();
    final map = await _channel.invokeMapMethod<Object?, Object?>('state');
    if (map == null) return const FlutroidState.unsupported();
    return FlutroidState.fromMap(map);
  }
