...
/**
 * The on-device half of Flutroid: decides which Dart snapshot the engine loads.
 *
 ...
 *
 * On-disk layout, all under `filesDir/flutroid`:
 * ```
 ...
 * ```
 */
object FlutroidPatch {
    private const val TAG = "Flutroid"
     ...
     *
     * Call from `MainActivity.getFlutterShellArgs()`. `flutroid init` writes
    ...
    private fun root(context: Context) =
        File(context.filesDir, "flutroid").apply { if (!isDirectory) mkdirs() }
